...
async function deploy(request: Request, env: Env) {
  ...
  return new Response(new ReadableStream({ async start(c) {
    ...
    try {
      ...
      const metadata = { main_module: 'worker.js', compatibility_date: '2026-08-26', assets: { jwt: assetsJwt, run_worker_first: ['/*'] }, bindings: [{ type: 'd1', name: 'DB', id: db.uuid }, { type: 'kv_namespace', name: 'KV', namespace_id: kv.id }, { type: 'queue', name: 'JOBS', queue_name: `${name}-jobs` }, { type: 'assets', name: 'ASSETS' }] }
